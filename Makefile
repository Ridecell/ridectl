...
build_macos:
	...
	GOOS=darwin GOARCH=amd64 go build -o bin/ridectl.macos -ldflags "-X github.com/Ridecell/ridectl/pkg/exec.tshMD5=$(shell md5sum pkg/exec/bin/tsh | awk '{ print $$1 }'| tr -d '\n') -X github.com/Ridecell/ridectl/pkg/cmd.version=$(shell git describe --tags)" -tags release github.com/Ridecell/ridectl/cmd/ridectl
...
build_linux:
	...
	cp teleport/tsh pkg/exec/bin/
	# Adding double $ in awk command to escape single $
	GOOS=linux GOARCH=amd64 go build -o bin/ridectl.linux -ldflags "-X github.com/Ridecell/ridectl/pkg/exec.tshMD5=$(shell md5sum pkg/exec/bin/tsh | awk '{ print $$1 }'| tr -d '\n') -X github.com/Ridecell/ridectl/pkg/cmd.version=$(shell git describe --tags)" -tags release github.com/Ridecell/ridectl/cmd/ridectl
